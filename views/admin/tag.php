...
<body class="d-flex align-items-center min-vh-100" style="background-color: #ddf3e5;">
            ...
            <div class="container-fluid">
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                    <div class="navbar-nav">
                        ...
                        <a class="nav-link" href="<?=\yii\helpers\Url::to(['admin/tag']);?>" style="text-decoration: underline;">Теги</a>
                        ...
                        <div class="modal-dialog modal-dialog-scrollable  modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Додати тег</h1>
    ...
    $this->registerJs("
        $(document).ready(function() {
            ...
            $.ajax({
                url: '". \yii\helpers\Url::to(['admin/tagout']) ."',
                type: 'post',
                dataType: 'json',
                ...
                success: function(response) {
                    ...
                    $.each(response.tags, function(index, tag) {
                        // Append a row to the table for each book
                        $('#t_body').append(
                            '<tr>' +
                                '<td>' + tag.id + '</td>' +
                                '<td>' + tag.name + '</td>' +
                                '<td><a href=\"' + '". \yii\helpers\Url::to(['admin/tag-info', 'id' => '']) ."' + tag.id +  '\"><img src=\"/img/info.png\" alt=\"View Details\" style=\"width:40px;height:40px;\" /></a></td>' +
                            '</tr>'
                        );
                    });
                }
    ...
    $('#add_b').click(function(e){
        ...
        $.ajax({
            url: '". \yii\helpers\Url::to(['admin/tag-add']) ."',
            type: 'post',
            dataType: 'json',
            data: {name: name},
            success: function(response) {
